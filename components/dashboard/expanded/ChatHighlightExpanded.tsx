...
const roleStyles: Record<string, string> = {
  ...
  subscriber: 'bg-gradient-to-br from-purple-950/95 to-purple-900/95 border-purple-500',
  moderator: 'bg-gradient-to-br from-gray-900/95 to-black/95 border-emerald-500',
  vip: 'bg-gradient-to-br from-pink-950/95 to-pink-900/95 border-pink-500',
  'first-timer': 'bg-gradient-to-br from-blue-950/95 to-blue-900/95 border-blue-500',
};
    ...
    <div className='bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50 shadow-xl'>
      ...
      {!isAuthenticated ? (
        <div className='text-center py-12'>
          ...
          <a
            ...
            className='inline-block bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-500 hover:to-purple-400 rounded-xl px-8 py-3 font-bold transition-all duration-200 shadow-lg hover:shadow-xl'
          >
            Connect with Twitch
          </a>
        </div>
        ...
        <>
          ...
          <div className='mb-4 bg-gradient-to-r from-purple-900/30 to-purple-800/30 rounded-xl p-4 border border-purple-500/30'>
            <div className='flex items-center gap-3'>
          ...
          {/* Current Highlight */}
          {currentHighlight && (
        <div className='mb-6 bg-gradient-to-br from-yellow-900/30 to-amber-900/30 rounded-xl p-4 border border-yellow-500/50'>
          <div className='flex items-center justify-between mb-3'>
            <div className='text-sm font-semibold text-yellow-400'>Currently Highlighted</div>
            <button
          ...
          <div className={`${roleStyles[currentHighlight.message.role]} border-l-4 rounded-lg p-3`}>
            <div className='flex items-start gap-3'>
              <span className='text-xl'>{roleIcons[currentHighlight.message.role]}</span>
              <div className='flex-1 min-w-0'>
                <div className='flex items-center gap-2 mb-1'>
                  <span
                    className='font-bold text-sm'
                    style={{ color: currentHighlight.message.color || '#ffffff' }}
                  >
                ...
                <p className='text-white text-sm'>{currentHighlight.message.message}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className='space-y-2 max-h-96 overflow-y-auto custom-scrollbar'>
          {recentMessages.length === 0 ? (
            ...
            recentMessages.map((message) => (
              ...
              >
                <div className='flex items-start gap-3'>
                  <span className='text-lg flex-shrink-0'>{roleIcons[message.role]}</span>
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-2 mb-1'>
                      <span
                      ...
                      <span className='text-xs text-gray-400 uppercase'>{message.role}</span>
      ...
      <div className='mt-6 pt-6 border-t border-gray-600'>
        ...
        <div className='grid grid-cols-2 gap-3 mt-3'>
          ...
          <div>
            <label className='block text-xs text-gray-400 mb-1'>
              Scale: {(componentLayouts.chatHighlight?.scale || 1).toFixed(1)}x
            </label>
            <input
              type='range'
              min='0.5'
          ...
          {/* Copy URL for OBS */}
          <CopyURLButton url={`${window.location.origin}/overlay/${sessionId}/chat-highlight`} />
        </>
      )}
